...
class IterableSnapshotDataset(torch.utils.data.IterableDataset):
    def __init__(self, time, horizon):
        ...
        self.time = time[sorted_idx]
        self.horizon = horizon
        self.num_workers = 1
        self.worker_id = 0
        self.time_min = int(self.time.min().floor())
        self.time_max = int(self.time.max().ceil())

    def __len__(self):
        return (self.time_max - self.time_min) // self.horizon + 1

    def __getitem__(self, index):
        mask = (self.time >= self.time_min + (index * self.horizon)) & (
            self.time <= self.time_min + ((index + 1) * self.horizon) - 1
        )
...
class SnapshotLoader(torch.utils.data.DataLoader):
    ...
    Args:
        time (FloatTensor): The time of each event.
...
def get_idx_data_loader(indices_list: list, time: torch.Tensor, batch_size: int, horizon: int, shuffle: bool):
    """
    get data loader that iterates over indices
    :param indices_list: list, list of indices
    :param time: torch.Tensor, time of each event
    :param batch_size: int, batch size
    :param horizon: int, time horizon for each snapshot
    :param shuffle: boolean, whether to shuffle the data
    :return: data_loader, DataLoader
    """
    if horizon > 0:
        data_loader = SnapshotLoader(
